"""
DB-WM vs E-GP / kernel observers on LST, one matched-protocol run.

LST-defaulted alias for :mod:`experiments.compare_v4`. It forces
``--modality lst`` and delegates; every flag of the underlying script is
accepted unchanged, so this is exactly::

    python -m experiments.compare_v4 --modality lst <same args>
...
Usage
-----
    python -m experiments.compare_lst_v4 --lst-dir /content/lst --weather-csv <csv> \\
...
from experiments.compare_v4 import main as _main


def main() -> None:
    """Delegate to :func:`experiments.compare_v4.main` with ``--modality lst``."""
    run(_main)
